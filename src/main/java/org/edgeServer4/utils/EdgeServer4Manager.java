...
public class EdgeServer4Manager {
    ...
    private static long compareComputeTime = 0; // 用于保存比较计算时间
    ...
    public static void processAggregatedCipherText(String cipherText) {
        System.out.println("edgeServer4开始解密聚合密文......\n密文：" + cipherText);
        if (!cipherText.isEmpty()) {
            try {
                ...
                System.out.println("edgeServer4解密完成......结果: " + decryptedText);
                ...
                // scaled.multiply(BigDecimal.TEN.pow(SCALE)).toBigInteger();
                // BigInteger encryptedValue = ImprovePaillier.encrypt(valueToEncrypt, 1); //
                // 使用SK_DO[1]进行加密
                // // 只保存，不自动上传
    ...
    public static void processVarianceData(String squareCipherText, int clientCount) {
        System.out.println("edgeServer4开始计算方差......sum^2密文: " + squareCipherText + ", 当前client数: " + clientCount);
        ...
        try {
            ...
            System.out.println("edgeServer4解密平方密文结束......sum^2结果 " + sumX2);
            ...
            if (meanValue != null && ex2Value != null) {
                ...
                System.out.println("edgeServer4计算E(x^2)-E(x)^2;E(x^2): " + ex2Value + ", E(x)^2: " + meanValueSquared);
                System.out.println("edgeServer4方差计算结束......方差结果" + varianceValue);
            } else {
                varianceValue = null;
            }
        } catch (Exception e) {
    ...
    public static void processMeanData(int clientCount) {
        System.out.println("edgeServer4开始计算均值......当前边缘节点接收到的密文总数: " + clientCount);
        if (clientCount > 0) {
            BigDecimal m = new BigDecimal(decryptedText);
            meanValue = m.divide(new BigDecimal(clientCount), 8,
                    RoundingMode.HALF_UP);
            System.out.println("edgeServer4计算均值完成......均值结果: " + meanValue);
        } else {
            meanValue = null;
        }
    }

    // 使用ImprovePaillier的SK_DO密钥对解密结果进行加密,发送给centerServer。
    ...
    // /get/meanResult时才计算均值:解密聚合密文->计算均值
    ...
    // /get/varianceResult时才计算方差：解密聚合密文->计算均值->解密平方密文->求方差
    ...
    public static String getCompareResult() {
        System.out.println(
                "edgeServer4: 获取极值结果, 用时: " + compareComputeTime + "ms");
    ...
    public static String getAndsendImpaillierVarianceCipherText() {
        System.out.println("edgeServer4开始加密平方和...");
        if (sumX2 == null) {
        ...
        return "边缘节点4对局部平方和的数据，使用ImPaillier进行二次加密。密文结果为:\n" + valueToEncrypt.toString() + "\n" + "当前接收到的密文数:" + savedClientCount+"\n"+"平方密文已顺利发送至中心服务器";
    }
}