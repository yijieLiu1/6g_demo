...
public class ServerMain {
    // 全局变量存储加密数据
    private static final List<String> encryptedDataList = new ArrayList<>();
    ...
    private static void startServer(int port) throws IOException {
        ...
        server.createContext("/get/sendData", new SendDataHandler());
        server.setExecutor(Executors.newFixedThreadPool(8));
        server.start();
    }

    static class SendDataHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                records = encryptedDataList.size();
                long endtime = System.currentTimeMillis();
                message = "已完成加密与发送，耗时:" + (endtime - starttime) + "ms，记录数:" + records;
    ...
    private static void outputEncryptedDataToCSV(String outputFilePath) {
        if (encryptedDataList.isEmpty()) {
            System.out.println("没有加密数据可输出，请先触发 /get/sendData 接口");
            return;
        }
