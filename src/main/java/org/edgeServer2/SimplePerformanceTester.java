...
public class SimplePerformanceTester {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=== EdgeServer2 API 性能测试 ===");
        System.out.println("测试次数: " + testCount);
        System.out.println();

        // 在执行所有测试之前，先执行edgeServer1的/get/sumcipherText
        System.out.println("准备数据：执行edgeServer1的聚合操作...");
        prepareData();
    ...
    private static void prepareData() {
        try {
            System.out.println("调用edgeServer1的/get/sumcipherText...");
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(EDGE_SERVER1_URL + "/get/sumcipherText"))
                    .GET()
                    .build();
            ...
            if (response.statusCode() == 200) {
                System.out.println("edgeServer1聚合操作成功完成");
                System.out.println("响应内容: " + response.body());
            } else {
                System.err.println("edgeServer1聚合操作失败，状态码: " + response.statusCode());
                System.err.println("错误信息: " + response.body());
            }
        } catch (Exception e) {
            System.err.println("执行edgeServer1聚合操作异常: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private static void testAPI(String endpoint, String description, int testCount) {
        ...
        for (int i = 1; i <= testCount; i++) {
            try {
                ...
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create(EDGE_SERVER2_URL + endpoint))
                        .GET()
                        .build();
    ...
    // 触发edgeServer1的计算
    private static void triggerEdgeServer1Computation() {
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(EDGE_SERVER1_URL + "/post/comparePair"))
            ...
            if (response.statusCode() != 200) {
                System.err.println("触发edgeServer1计算失败，状态码: " + response.statusCode());
            }
        } catch (Exception e) {
            System.err.println("触发edgeServer1计算异常: " + e.getMessage());
        }
    }
